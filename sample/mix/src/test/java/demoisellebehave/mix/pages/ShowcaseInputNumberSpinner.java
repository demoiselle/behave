package demoisellebehave.mix.pages;

import br.gov.frameworkdemoiselle.behave.annotation.ElementLocatorType;
import br.gov.frameworkdemoiselle.behave.annotation.ElementMap;
import br.gov.frameworkdemoiselle.behave.annotation.ScreenMap;
import br.gov.frameworkdemoiselle.behave.runner.webdriver.ui.richfaces4.RichFileUpload;
import br.gov.frameworkdemoiselle.behave.runner.webdriver.ui.richfaces4.RichInputNumberSpinner;

@ScreenMap(name = "Showcase inputNumberSpinner", 
	location = "http://showcase.richfaces.org/richfaces/component-sample.jsf?demo=inputNumberSpinner&skin=blueSky")
public class ShowcaseInputNumberSpinner {

	@ElementMap(name = "inputNumberSpinner", locatorType = ElementLocatorType.Id, locator = "j_idt1481")
	private RichInputNumberSpinner input;

}
