...
public class WebTextField extends WebBase implements TextField {
	...
	public void sendKeysWithTries(CharSequence... keysToSend) {
		...
		while (!elements.get(0).getAttribute("value").equals(value)) {
			...
			String finalValue = Keys.chord(Keys.CONTROL, "a") + Keys.chord(Keys.DELETE) + value + Keys.chord(Keys.CANCEL);

			// Se for Chrome o último caracter deve ser ESCAPE e não CANCEL
			if (caps.getBrowserName().equals("chrome")) {
				finalValue = Keys.chord(Keys.CONTROL, "a") + Keys.chord(Keys.DELETE) + value + Keys.chord(Keys.ESCAPE);
			}
