...
var getNodeByPath = function (w,path) {
	var node = null;
	var nodeList = w.jq.children('ul.ui-tree-container').children('li.ui-treenode');
	var sPath = path.split('.');
	for( var i in sPath ) {
		var n = parseInt( sPath[i] );
		if( n > 0 && n <= nodeList.length ) {
			node = jQuery( nodeList.get( n-1 ) );
			nodeList = node.children('ul.ui-treenode-children').children('li.ui-treenode')
		} else return null;
	}
	return node;
}
...
var getNodeByLabel = function (w,label) {
	...
	for( var i in nodeList) {
		...
		if( label.trim() == node.children('span.ui-treenode-content').children('span.ui-treenode-label').text().trim() ) {
			return node;
		}
	}
	return null;
