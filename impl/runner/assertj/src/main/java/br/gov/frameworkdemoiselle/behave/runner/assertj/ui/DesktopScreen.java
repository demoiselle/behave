...
public class DesktopScreen extends DesktopBase implements Screen {
	...
	private BehaveMessage message = new BehaveMessage(AssertJRunner.MESSAGEBUNDLE);
	...
	public void waitNotText(String text) {
		...
		while (!waitTimeExpired) {
			...
			waitThreadSleep(BehaveConfig.getRunner_ScreenMinWait());
			waitTimeExpired = (GregorianCalendar.getInstance().getTimeInMillis() - startedTime) > BehaveConfig
	...
	private boolean findText(String text) {
		try {
			return runner.getHierarchy().replaceAll("\n", "").contains(text);
